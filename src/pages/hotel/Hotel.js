...
const Hotel = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get("https://localhost:3000/api/hotels");
  ...
  const fetchData2 = async () => {
    try {
      const response = await axios.get("https://localhost:3000/uploads/updImage");
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`https://localhost:3000/api/hotels/${id}`);
